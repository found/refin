module ActiveMerchant #:nodoc:
  module Billing #:nodoc:
    ...
    class AuthorizeNetCimGateway < Gateway
      ...
      puts "In Authorize.NET : #{Rails.env}"
      if Rails.env == "development"
        self.live_url = 'https://apitest.authorize.net/xml/v1/request.api'
      else
      ...
      def create_customer_payment_profile(options)
        ...
        #@kevin this is the culprit line below.
      ...
      def build_create_customer_profile_request(xml, options)
        #@kevin down the rabbit hole.
        Rails.logger.info("Inside build_create_customer_profile_request")
        Rails.logger.info("#{xml.inspect}")
        Rails.logger.info("#{options[:profile].inspect}")
        add_profile(xml, options[:profile])
        Rails.logger.info("add_profile isn't the culprit")
        xml.target!
        Rails.logger.info("At the end")
      end
      ...
      def build_create_customer_payment_profile_request(xml, options)
        ...
        xml.tag!('paymentProfile') do
          # Rails.logger.info("In the iterator #{options[:payment_profile]}")
          add_payment_profile(xml, options[:payment_profile])
        end
        Rails.logger.info("outside of iterator.  validation_mode: #{options[:validation_mode].inspect}")
      ...
      def add_payment_profile(xml, payment_profile)
        ...
        if payment_profile[:bill_to]
          Rails.logger.info("we gotta bill to")
          xml.tag!('billTo') do
            # Rails.logger.info("in the iterator here: #{payment_profile[:bill_to]}")
            add_address(xml, payment_profile[:bill_to])
          end
        end
        Rails.logger.info("outside.")
        Rails.logger.info("#{payment_profile[:payment].inspect}")
        if payment_profile[:payment]
          Rails.logger.info("payment is true.  adding the info. CC#{payment_profile[:payment].has_key?(:credit_card)} or bank #{payment_profile[:payment].has_key?(:bank_account)}")
          xml.tag!('payment') do
            Rails.logger.info("Oh skrit.  the problem is in here!")
            add_credit_card(xml, payment_profile[:payment][:credit_card]) if payment_profile[:payment].has_key?(:credit_card)
            Rails.logger.info("Check 1")
            add_bank_account(xml, payment_profile[:payment][:bank_account]) if payment_profile[:payment].has_key?(:bank_account)
            Rails.logger.info("Check 2")
            add_drivers_license(xml, payment_profile[:payment][:drivers_license]) if payment_profile[:payment].has_key?(:drivers_license)
            Rails.logger.info("This too shall pass.")
            ...
            #Rails.logger.info("Not using wells fargo, dont need tax id.")
          end
        end
        Rails.logger.info("bada bing.  We're on the outside.")
      ...
      def add_bank_account(xml, bank_account)
        Rails.logger.info("Skrit 2.  now we in here nugs")
        ...
        Rails.logger.info("commented out some jank")
        Rails.logger.info("Bout to tagg")
        xml.tag!('bankAccount') do
          Rails.logger.info("Inside bankAcccount")
          # The type of bank account
          Rails.logger.info("1: #{BANK_ACCOUNT_TYPES.inspect} and #{bank_account.inspect}")
          xml.tag!('accountType', BANK_ACCOUNT_TYPES[bank_account.account_type])
          # The routing number of the customer’s bank
          Rails.logger.info("2: #{bank_account.inspect}")
          ...
          Rails.logger.info("5")
          #xml.tag!('echeckType', ECHECK_TYPES[bank_account.echeck_type]) if bank_account.echeck_type
          # The full name of the individual associated 
          # with the bank account number (optional)
          #xml.tag!('bankName', bank_account.bank_name) if bank_account.bank_name
          Rails.logger.info("I'm out")
        end
      end
