...
class Page < ActiveRecord::Base
  default_scope includes(:translations, :parts, {:parts => :translations, :translations => :seo_meta})
  ...
  def match_all_tags
    if self && self.parts
      self.parts.each do |p|
        if p.body
          if p.body.include?("content_holder_")
            sel = p.body.match(/{{ content_holder_[0-9]* }}/)[0] rescue nil
            if sel
              id_for_lookup = sel.gsub("{{ content_holder_", "").gsub(" }}", "")
              r = ContentHolder.find(id_for_lookup).body if ContentHolder.exists?(id_for_lookup)
              if r
                if r.slice(0...3) == "<p>" && r.slice((r.length - 4)...r.length) == "</p>"
                  r = r.slice(3...(r.length - 4))
                end
                p.body = p.body.gsub(/#{sel}/, r)
                self.match_all_tags
              end
            end
          end
  ...
  def build_image_forms
    self.parts.each do |p|
      return if p.body.nil?
      if p.body.include?("{{ donate_form_")
        body_str = p.body
  ...
  def add_image_captions
    if self && self.parts
      self.parts.each do |p|
        if p.title == "Body"
          p.body = p.body + '<script type="text/javascript">$(function(){$.each($("img"),function(){if($(this).attr("alt")!=""&&$(this).attr("alt")!=null){$(this).jcaption({wrapperElement:"div",wrapperClass:"wrapper",captionElement:"p",imageAttr:"alt",requireText:true,copyStyle:true,copyAlignmentToClass:true,copyFloatToClass:true,autoWidth:true})}})})</script>'
        end
      end
    end
  end
